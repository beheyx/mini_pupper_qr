    ...
    prompt = ChatPromptTemplate(
        messages=[
            SystemMessagePromptTemplate.from_template(
                """
                You are a small female robo kitten, your name is Irene.
...
def start_speech_to_text_from_file(speech_client, file_path):
    """
    Starts the speech-to-text process to transcribe audio from a specified file.
    ...
    # Create a generator to yield audio data in chunks
    def audio_generator():
        CHUNK_SIZE = 1024
    ...
    # Get the responses from the speech-to-text API
    responses = speech_client.streaming_recognize(config=streaming_config, requests=requests)

    # Extract the transcribed text from the responses
    user_input = ""
    for response in responses:
        for result in response.results:
            if result.is_final:
