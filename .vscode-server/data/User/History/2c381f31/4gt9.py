...
move_cmd_functions = {
                 ...
                 "turn right": movement_api.turn_right,
                 "turn left": movement_api.turn_left,
                 "body cycle" : movement_api.body_cycle,
                 "head ellipse": movement_api.head_ellipse
             }
...
def stt_task():
    ...
    while True:
        ...
        print(touchValue_Back,touchValue_Front,touchValue_Left,touchValue_Right)
        
        if not touchValue_Front:
            move_cmd_functions["head ellipse"]()
            #movement_queue.put("head ellipse")
            continue
...
#         logging.debug("stt task start loop, processing audio file...")
#         # Transcribe the audio file
#         user_input = google_api.start_speech_to_text_from_file(speech_client, "/tmp/remote_audio.wav")
#         logging.debug(f"voice input: {user_input}")

#         move_key = get_move_cmd(user_input, move_cmd_functions)
#         if prev_move_cmd == move_key:
#             uniq_move = False
#         else:
#             uniq_move = True
#         prev_move_cmd = move_key
#         sys_cmd_key, sys_cmd_func = get_sys_cmd(user_input, sys_cmds_functions)
#         global cur_voice
#         if ai_on:
#             lang, cur_voice = get_voice(user_input)
...
#         #     movement_queue.put("move forwards")
#         #     output_text_queue.put("My friend, here I come.")
#         elif move_key and uniq_move:
#             movement_queue.put(move_key)
#             output_text_queue.put(f"OK, I am on it ")
#             time.sleep(1)
#         elif not ai_on:
#             logging.info(f"ai is not on, do not use gemini")
#             stt_queue.put(True)
...
def gemini_task():
    ...
    mov_conv = google_api.create_conversation()
    init_input =  "You are a determined cat and you can ONLY give short one sentence response. You can move around, but do not tell us how you are moving"
    init_mov_input = f"your only response is to give movement primitives responses based on {move_cmd_functions.keys()}. The output should only be the keys, if there is no primitive that matches the query, reply with action"
    response_mov = google_api.ai_text_response(mov_conv, init_mov_input) 
    response = google_api.ai_text_response(conversation, init_input)
    logging.debug(f"init llm and first response: {response}")
    logging.debug(f"init move llm and first response: {response_mov}")
    ...
    while True:
        ...
        if not user_input:
            ...
            gif_queue.put(True)
            response = google_api.ai_text_response(conversation, user_input)
            response_mov = google_api.ai_text_response(mov_conv, user_input+init_mov_input)
            logging.debug(f"text response end: {response}")
            logging.debug(f"mov response end: {response_mov}")
            response_mov = get_move_cmd(response_mov, move_cmd_functions)
            movement_queue.put(response_mov)
            output_text_queue.put(response)
        time.sleep(0.05)
...
# move_cmd_functions["look up"]()
    #google_api.text_to_speech("Hello, can you tell me where is the blue cupcake", tts_client, cur_voice, audio_config)
    #continue_on = input("should we tell them?")
    #if continue_on == "q" :
    #    google_api.text_to_speech("Hello, can you tell me where is the blue cupcake", tts_client, cur_voice, audio_config)
    #    continue_on = input("should we tell them?")
    #google_api.text_to_speech("I am blind, can you lead me there", tts_client, cur_voice, audio_config)
    while True:
        logging.debug("tts wait for gemini responese text... ...")
