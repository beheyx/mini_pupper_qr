...
def gemini_task():
    ...
    while True:
        ...
        if not user_input:
            ...
            # output_text_queue.put(response)
            prompt = f"You are a happy dog. Given the user's command : {user_input}, provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send null"
