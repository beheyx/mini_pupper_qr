...
def gemini_task():
    ...
    # Structured request to Gemini, asking for both the response and movement primitive
    prompt = f"You are a happy dog. Given the user's command, provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send null"
    conversation = google_api.create_conversation()
    ...
    while True:
        ...
        if not user_input:
            ...
            structured_output = google_api.ai_text_response(conversation,user_input+prompt)
    # Log the raw output for debugging
