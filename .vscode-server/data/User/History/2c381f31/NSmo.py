...
def gemini_task():
    ...
    # Structured request to Gemini, asking for both the response and movement primitive
    prompt = f"You are a happy dog. Given the user's command, provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as a dictionary as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send action"
    conversation = google_api.create_conversation()
