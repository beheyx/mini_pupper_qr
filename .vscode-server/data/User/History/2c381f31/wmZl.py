...
from api import media_api, google_api, move_api, shell_api,movement_api
...
voice0 = texttospeech.VoiceSelectionParams(language_code="en-US", name="en-US-Journey-F")
...
move_cmd_functions = {
                 ...
                 #"sit": movement_api.sit,
                 "move forward": movement_api.move_forward,
                 "move backward": movement_api.move_backward,
                 "move left": movement_api.move_left,
                 "move right": movement_api.move_right,
                 "look up": movement_api.look_up,
                 "look down": movement_api.look_down,
                 "look left": movement_api.look_left,
                 "look upper left": movement_api.look_upperleft,
                 "look lower left": movement_api.look_leftlower,
                 "look right": movement_api.look_right,
                 "look upper right": movement_api.look_upperright,
                 "look lower right": movement_api.look_rightlower,
             }
...
def get_move_cmd(input_text, command_dict):
    ...
    # Structured request to Gemini, asking for both the response and movement primitive
    prompt = f"Given the user's command: '{input_text}', provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send None"
    conversation = google_api.create_conversation()
    # Call Gemini to get the structured response
    structured_output = google_api.ai_text_response(conversation,prompt)

    try:
        # Parse the structured response
        response_dict = json.loads(structured_output)
        movement_primitive = response_dict.get("movement primitive", None)
        return movement_primitive
...
def stt_task():
    ...
    device_index = 2  # Set the correct index from the list_audio_devices() output
    # Modify the audio stream to use the specified input device
    ...
    while True:
        ...
        user_input, _ = google_api.start_speech_to_text(speech_client, py_audio)
...
def gemini_task():
    ...
    """
    command_dict = move_cmd_functions
    logging.debug("gemini task start.")

    # Structured request to Gemini, asking for both the response and movement primitive
    prompt = f"You are a happy dog. Given the user's command, provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as a dictionary as: response: \<Your response>\, \movement primitive\: \<Command primitive>\. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send action"
    conversation = google_api.create_conversation()
    ...
    logging.debug(f"Raw structured output from Gemini: {structured_output}")
    ...
    try:
        ...
        logging.debug(f"Parsed movement primitive: {movement_primitive}")

        # Return the movement primitive
        #return movement_primitive

    except (json.JSONDecodeError, TypeError) as e:
        logging.error(f"Failed to parse Gemini response: {e}")
        logging.error(f"Problematic structured output: {structured_output}")
        #return None
    ...
    while True:
        ...
        if not user_input:
            ...
            if image:
                ...
                user_input += "describe the color without mentioning the color name"
            ...
            # output_text_queue.put(response)
            #prompt = f"You are a happy dog. Given the user's command : {user_input}, provide the appropriate movement primitive based only and only on the available commands: {list(command_dict.keys())}. Please structure the response as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. If you think no motion is required, send action"

            structured_output = google_api.ai_text_response(conversation,user_input)
    # Log the raw output for debugging
            structured_output = structured_output.replace("None", "null")
            print(f"output from llm was : {structured_output}")
            logging.debug(f"Raw structured output from Gemini: {structured_output}")
            response_part = structured_output.split("response:", 1)[1].split("movement primitive:", 1)[0].strip()
            #response = response_part

        # Extract the 'movement primitive' part
            movement_primitive_part = structured_output.split("movement primitive:", 1)[1].strip()
            #movement_primitive = movement_primitive_part
            output_text_queue.put(response_part)
            movement_queue.put(movement_primitive_part)
            #     movement_queue.put(movement_primitive)
            #logging.debug(f"response was: {structured_output.response}")
            # try:
            #     # Parse the structured response
            #     response_dict = json.loads(structured_output)
            #     response = response_dict.get("response", None)
            ...
            #     output_text_queue.put(response)
            #     prompt = f"You are a happy dog. Given the user's command, provide the appropriate movement primitive based on the available commands: {list(command_dict.keys())}. Please structure the response as:\n\n{{\"response\": \"<Your response>\", \"movement primitive\": \"<Command primitive>\"}}. Movement primitive can only and only include commands based on {list(command_dict.keys())}. If you think no motion is required, send action"
            #     conversation = google_api.create_conversation()
            #     structured_output = google_api.ai_text_response(conversation, prompt)
